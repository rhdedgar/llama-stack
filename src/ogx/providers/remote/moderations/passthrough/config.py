...
class PassthroughProviderDataValidator(BaseModel):
    """Validates provider-specific request data for passthrough moderations forwarding."""

    model_config = ConfigDict(extra="allow")

    passthrough_api_key: SecretStr | None = Field(
        default=None,
        description="API key for the downstream moderations service",
    )


@json_schema_type
class PassthroughModerationsConfig(BaseModel):
    """Configuration for the passthrough moderations provider that forwards to a downstream service."""

    model_config = ConfigDict(extra="forbid")
    base_url: HttpUrl = Field(
        description="Base URL of the downstream moderations service (e.g. https://moderations.example.com/v1)",
    )
    api_key: SecretStr | None = Field(
        default=None,
        description="API key for the downstream moderations service. If set, takes precedence over provider data.",
    )
    ...
    def validate_forward_headers(self) -> "PassthroughModerationsConfig":
    ...
    def sample_run_config(
        cls,
        base_url: str = "${env.PASSTHROUGH_MODERATIONS_URL}",
        api_key: str = "${env.PASSTHROUGH_MODERATIONS_API_KEY:=}",

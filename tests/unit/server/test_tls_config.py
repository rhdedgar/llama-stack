...
class TestSecurityModeValidation:
    ...
    def test_production_mode_auto_populates_ciphers_when_tls_config_has_none(self):
        ...
        config = ServerConfig(
            ...
            tls_certfile="/path/to/cert.pem",
            tls_keyfile="/path/to/key.pem",
            tls_config=ServerTLSConfig(min_version="TLSv1.3"),
        )
        assert config.tls_config.ciphers == FIPS_APPROVED_CIPHERS
...
class TestInsecureFlagOverride:
    def test_insecure_flag_overrides_production(self):
        """The --insecure flag should allow creating a development config even when production was set."""
        # Simulate what the CLI does: create config, then override
        config = ServerConfig(
            security_mode=SecurityMode.DEVELOPMENT,  # --insecure overrides to this
            tls_certfile="/path/to/cert.pem",
            tls_keyfile="/path/to/key.pem",
        )
        assert config.security_mode == SecurityMode.DEVELOPMENT
...
class TestHSTSMiddleware:
    def test_hsts_header_present_with_tls(self):
        """HSTS header should be added when HSTSMiddleware is active."""
        from llama_stack.core.server.server import HSTSMiddleware
